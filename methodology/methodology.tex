...
where $F$ is the force applied, $k$ is the stiffness of a small element, and $u$ is the displacement of the element from its equilibrium position. For finite element analysis, the minimization of the compliance $c$ of the structure and its mechanical behavior can be expressed as:
...
\begin{table}[h!]
  ...
  \begin{tabular} { |c | c| }
    ...
    Density & 8.0 g / cm$^3$ \\
    \hline
    Thermal conductivity & 15 W / (m$\cdot$K) \\
    \hline
    Heat capacity & 500 J / (kg$\cdot$K) \\
    \hline
...
Finally, the objective functions weights were also varied for the design of topologies for the simple geometry parts. The pairings of weight used were (0.2, 0.8), (0.5 , 0.5), and (0.2, 0.8), where the first weight in each pair corresponds to the weight of the thermal compliance objective and the second corresponds to the weight of the mechanical compliance objective. All of these parameters are summarized below in Table \ref{tab:params}.
...
\begin{table}
  ...
  Parameter & Values \\
  \hline
  Volume fraction & asdf \\
  \hline
  Hyperbolic tangent angle & asdfasd \\
  \hline
  Objective weights & adfasdf \\
  \hline
\end{tabular}
...
\subsection{Toberenamed: final comsol details}

\todo{Explain here and show the setup of comsol simulation.}
